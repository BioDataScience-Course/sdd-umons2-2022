...
Un nouveau script s'ouvre avec le coce suivant :
...
Nous effectuons d'abord le traitement à partir du tableau `wol` non retravaillé (section facultative) afin de démontrer les difficultés qui apparaissent lorsque les données sont mal présentées. Ensuite, nous remanierons le tableau `wol` et feront le traitement à nouveau. En R, nous pourrions utiliser les fonctions "tidy" ou "speedy" (et d'autres encore) pour décomposer le problème en plusieurs étapes plus simples. Nous utiliserons ici autant que possible les fonctions "tidy". *Réfléchissez un peu par vous-mêmes comment vous feriez pour répondre à cette question dans R avant de continuer votre lecture...*

<details>

<summary>Traitement à partir du tableau `wol` non retravaillé</summary>

Nous pouvons dé"composer le problème comme suit :
...
Les étapes 3 et 4 sont mises en évidence car elles seront difficile à réaliser à cause du mauvais encodage. C'est parce que l'information requise (genre et classe d'âge) n'est **pas** reprise sous forme de données dans le tableau, mais **cachée** dans le nom des colonnes. Les deux premières étapes sont faciles pour quelqu'un qui maîtrise un peu les fonctions "tidy".
...
Les étapes suivantes ne posent pas pas problèmes particuliers car nous venons d'évacuer la difficulté du mauvais encodage des données avec le calcul de notre colonne `all_new`.
...
Ouf ! Cela n'a pas été simple du tout. Faites une pose bien méritée, mais ne vous découragez pas si vous avez perdu le fil. L'important est de tirer la conclusion que, lorsque les données sont mal encodées, le traitement risque de s'avérer très pénible même dans R.
...
whol %>.%
  ...
  summarise(., total = population, all_new = sum(new_cases, na.rm = TRUE)) %>.% # étape 4 somme des cas
  distinct(.) %>.% # 12 fois les mêmes lignes, n'en garder qu'une à chaque fois
