...
Ensuite seulement, vous pourrez lire le jeu de données avec l'instruction `read()`un peu plus haut. Pour visionner de manière compacte la structure du tableau, vous pouvez utiliser `str()` :
...
Notez à présent la quantité de mémoire utilisée par votre session R... Normalement, si vous êtes parti d'une session vide, vous restez encore largement en dessous du gigaoctets, soit très loin des 16Go disponibles. On a de la marge et aucune précaution particulière ne doit être prise avec des tableaux "aussi petits" pour R. Par contre, faites bien attention aux outils statistiques que vous utilisez dessus. Par exemple, ajuster un modèle linéaire généralisé avec variable aléatoire dans une telle quantité de données prendra soit un temps très, très long, soit fera planter la machine par manque de mémoire (il s'agit d'une technique qui demande énormément de calculs).
...
Nous faisons maintenant de même pour les trois format de compression **.gz**, **.bz2** et .**xz**. Compression GZ :
...
Vous pouvez naviguer jusque votre dossier `babynames_to_delete` dans l'onglet **Files** pour voir vos différents fichiers. Enfin, lorsque vous avez fini de travailler avec des gros jeux de données, pensez à nettoyer votre environnement de travail, et éventuellement, vos dossiers des objets et fichiers devenus inutiles (**important sur SaturnCloud !**).
...
Troisièmement, vous utilisez `read()` sur l'URL et indiquez le fichier désiré dans `cache_file=`. Cela donne quelque chose comme ceci (spécifiez toujours bien le format avec `$…` si vous charger depuis une URL) :
