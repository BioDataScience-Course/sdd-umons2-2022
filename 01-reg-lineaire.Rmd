...
models1 <- fmutate(models1, fobj = purrr::map2_dbl(intercept, slope, f_obj_trees))
```
...
plot <- chart(data = models1, slope ~ intercept %col=%  fobj) +
  geom_point() +
  geom_point(data = fsubset(models1, rank(fobj) <= 10),
...
```{r}
best_models <- fsubset(models1, rank(fobj) <= 10)
```
...
[^01-reg-lineaire-3]: Lors de la paramétrisation du modèle, pensez à arrondir la valeur des paramètres à un nombre de chiffres significatifs raisonnables. Inutile de garder 5, ou même 3 chiffres derrière la virgule si vous n'avez que quelques dizaines d'observations pour ajuster votre modèle.
...
```{r}
new <- dtx(diameter = seq(0.1, 0.7, length.out = 8))
```
...
```{r}
new <- modelr::add_predictions(new, lm.)
new
```
...
```{r}
trees <- modelr::add_predictions(trees, lm.)
