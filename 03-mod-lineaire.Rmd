...
babies %>.% select(., wt, wt1, smoke) %>.% # Garder seulement wt, wt1 & smoke
  ...
  mutate(., smoke = as.factor(smoke)) %->% # S'assurer d'avoir une variable factor et collecter avec %->%
  Babies # Enregistrer le résultat dans Babies

knitr::kable(head(Babies))
```

Notez ici l'utilisation de l'**assignation alternative** `%->%` à la place de `->`. Cette variante de l'assignation standard permet de **collecter** les résultats dans la nouvelle version de `SciViews::R`. En effet, les pipelines qui font intervenir des fonctions dite "tidy" comme `select()`, `mutate()`, `filter()`, `group_by() |> summarise()` construisent maintenant des étapes d'analyses qui sont au final combinées en un code plus performant à l'aide du package {dtplyr} en travaillant sur les data frames de classe **data.table** qui sont à présent utilisées.
